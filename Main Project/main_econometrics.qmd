...
The groups here are determined using the cut_number function from ggplot creaing groups with approximately equal observations.

While ppm2 clearly interacts somewhat with pphh, in the log of ppm2 lowest pphh group loses much of its interaction. While the interaction term with the unlogged version is significant in the model this is clearly due to spurious correlation with a complete lack of linearity in the predictors. The logged term becomes insignifgiant in the model which is also telling. This is the same for the high estimate between officers and the unlogged ppm2.

The remaining interaction is the one between reservations and net migration, which has none of the problems with linearity and does not require a transformation.
...
```
Could be indicative of population density but when mmigration interactions with ppm2, pct non white, median hh income, and log_per_capita_alc_taxes is included reservations interaction remains significant and ppm2 remains insignificant. Even when the plot is cut off at .1 pcm to remove the potential outliers/ points heavily influencing the trend, the interaction remains strong

This interaction shows that higher levels of migration lead to higher levels of crime in counties with reservations than in counties without.
...
modeling_vars <- c( "median_household_income_acs5", "log_pct_non_white", "log_persons_per_m2",
  "reservations", "log_per_capita_alc_taxes", "reservations:per_capita_migration"
)
...
model_tidy_with_robust <- function(model) {
  ...
  # Robust (HC3) tidy
  robust_vcov <- vcovCL(model, cluster = acs_5_yearly_data$county)
...
Acknowledge the spacial/temporal autocorrelation and attempt to fix with robust errors. A much mor erestrictive diff model will also be fit to completly remove these problems and examine how the changes in crime are impacted by the changes in these variables.

I should try various errors, using cr1 - liang zeger now, try out cr2, cr3 and compare to hc3

<https://www.nber.org/system/files/working_papers/w24003/w24003.pdf> - generally talks about when to cluster. Says that even cr1 is conservative and generally suggests that using errors robust to clustering is not always necessary

      <!-- The resulting model reveals multiple noteworthy relationships between county-level crime and the sociological factors outlined in Norman, J. Mark and Arwood, Donald E’s study \[1\]. In the above model log_officers, top_cities, pct_highschool_or_greater, and all of their interaction terms were removed due to insignificance. The main variables significant at a <.01 level being median household income, persons per m^2^, the indicator for reservations, and per capita alcohol taxes. The rest of the variables in the model seem to be relatively insignificant at predicting crime rates besides the log of the percent of non white people in a county. Furthermore, almost every interaction term included in the model as well as the post COVID indicator variable remain insignificant at the >.5 level. This result shows a drastic change from Norman and Arwood's previous findings and also shows that despite the spike in crime rates during COVID these impacts have not had a large impact on how crime relates to the traditional sociological predictors of crime. -->
...
modeling_vars <- c("crime_rate_sqrt", "log_persons_per_m2")
...
modeling_vars <- c("crime_rate_sqrt", "median_household_income_acs5", "log_pct_non_white", 
                    "log_persons_per_m2", "log_per_capita_alc_taxes")
...
modeling_vars <- c( "median_household_income_acs5_diff", "log_pct_non_white_diff", 
                    "log_persons_per_m2_diff", "log_per_capita_alc_taxes_diff")
...
Nothing here except for with the ppm2, almost impressive lack of correlation
