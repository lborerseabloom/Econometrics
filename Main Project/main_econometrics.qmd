...
plot(total_model, 1)
```
      The full model reveals a limited set of robust predictors of county-level crime, with many traditional sociological variables losing explanatory power once controls and interaction terms are included. Consistent with Norman and Arwood's previous work, only a small subset of variables remain statistically significant at conventional levels, as well as most interaction terms with the post-COVID indicator not being statistically distinguishable from zero.\
...
      Fitting a model using the difference between pre and post COVID to see if any of the changes in explanatory variables are associated with a change in crime rates. Using this approach, the only two variables found to have a statistically significant relationship on the change in crime rates are the logarithm of the percentage of the population identified as white and the population density measured in m^2^. This contrasts with the conclusions of the previous linear model as the previously significant and meaningful income predictor shows no relationship. The significance of the percent non-white variable and population density in this model seem to be more representative of the areas which were most affected by the pandemic, such as urban centers and reservations, rather than a direct impact.
```{r differenced_data}
...
      Overall, the findings from this study suggest that while crime rates fluctuated during the COVID-19 pandemic, the determinants of crime in Minnesota have remained largely stable. The difference model indicates that pandemic-related shifts in population composition and density account for some of the variation in crime changes, but the core relationships between crime and its economic, demographic, and geographic predictors remain consistent both over time and relative to the findings of Norman and Arwood’s 1998 study [1].
