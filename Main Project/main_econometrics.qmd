...
# my github needs a real citation
# facet wrap or something the previous distriution
# further LASSO discussion, citations, and equation
# remove unlogged variables from LASSO and drop disscussion of variance and interaction
# citations and better order to structural break tests
# evaluate conditions of diff model and if sqrt is needed
# better in text and bottom citations
-->
```
...
acs_5_yearly_data|>
  ...
  geom_density(aes(x=crime_rate, y = ..density.. * 0.2, fill = "Untransformed"), 
               alpha = .25,
...
      This interpretation is further supported by the structural break tests presented below, which fail to detect meaningful parameter instability over time. Two complementary tests are employed: the Chow (M-fluctuation) test and the CUSUM test. The Chow test explicitly evaluates whether the regression coefficients differ across a predefined breakpoint, in this case the onset of the COVID-19 period. It tests the null hypothesis that the relationship between crime and its explanatory variables remains constant before and after the break. The results of the Chow test fail to reject this null hypothesis, indicating no statistically significant structural change in the estimated coefficients following the onset of COVID.\
      The CUSUM test provides a more flexible assessment of parameter stability by examining the cumulative sum of recursive residuals over time. Rather than testing for a break at a single known point, the CUSUM test detects gradual or unknown structural changes by identifying whether the estimated parameters drift outside their confidence bounds. In the present analysis, the CUSUM statistics remain well within the critical boundaries throughout the sample period even during the shift from pre to post COVID, suggesting stable regression parameters over this period. This is seen in the failure to reject the null hypothesis that the variance in the estimated parameters is stable over time.\
...
$$
\begin{aligned}
\sqrt{crime\_rate\_post} - \sqrt{crime\_rate\_pre} 
&= \hat{\beta}_0
+ \hat{\beta}_1 \, (\log(persons\_per\_m2\_post) - \log(persons\_per\_m2\_pre)) + \\
&\quad \hat{\beta}_2 \, (pct\_young\_males\_post - pct\_young\_males\_pre) + \varepsilon
\end{aligned}
$$
