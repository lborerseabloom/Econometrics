...
homicide_files <- files[!grepl("_", files)]
officer_files <- files[grepl("_", files)]

homicide_data <- tibble(series = c(), month = c(), homicides = c(), ori = c())
for(file_path in homicide_files){
...
homicide_data <- left_join(homicide_data, officer_counts, by = join_by(ori, year))
```
...
cor(
  acs_1_yearly_data |> 
    ...
    select(homicides, periods, post_covid, officers, median_household_income, pct_lessthan_5kincome, pct_highschool_or_greater, pct_children_missing_parents),
