...
yearly_ts <- homicide_data |>
  ...
  summarise(homicides = sum(homicides), .groups = "drop") |>
  mutate(
    month = as.Date(paste0(year, "-01-01")),  # tsibble yearmonth
    type = "Yearly"
  ) |>
  select(month, homicides, type)
...
yearly_large_county_data|>
  ...
  geom_point()+
  theme_minimal()+
  labs(title = "Minnesota Homicides per Capita Over Time", x = "Date", y = "Homicides per Capita")
