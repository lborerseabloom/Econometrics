...
a <- mapping_yearly_data |>
  ...
  scale_fill_viridis_c(
    ...
    labels = scales::label_number(scale_cut = scales::cut_short_scale())  # e.g., 10K, 1M
  ) +
  theme_minimal() +
  theme(
...
modeling_vars <- c("log_officers", "median_household_income_acs5", "pct_lessthan_5kincome", 
...
combined <- bind_rows(
  tidy(pre_covid_model)  |> mutate(Model = "Pre-COVID"),
  tidy(post_covid_model) |> mutate(Model = "Post-COVID"),
  tidy(total_model)      |> mutate(Model = "Full Model")
)|>
  select(Model, term, estimate, std.error, p.value) |>
  mutate(across(c(std.error, p.value), ~ round(., 3)))

pre_rows  <- 1:nrow(tidy(pre_covid_model))
post_rows <- (max(pre_rows)+1):(max(pre_rows)+nrow(tidy(post_covid_model)))
full_rows <- (max(post_rows)+1):(max(post_rows)+nrow(tidy(total_model)))

# Create the table with grouped rows
kbl(combined, caption = "Regression Models with Grouped Rows") |>
  kable_styling("striped", full_width = FALSE) |>
  group_rows("Pre-COVID", pre_rows[1], pre_rows[length(pre_rows)]) |>
  group_rows("Post-COVID", post_rows[1], post_rows[length(post_rows)]) |>
  group_rows("Full Model", full_rows[1], full_rows[length(full_rows)])
